...
fn render_text(node_storage: frontend::Storage<frontend::Immutable>) -> Vec<TextType> {
...
fn compute_string(string: String) -> Option<f32> {
    let splited = string.split_whitespace().collect::<Vec<String>>();
...
fn main() {
    let mut node_storage = frontend::Storage::new();
    node_storage.put_mut(Selected(KeyPads::Zero));
    node_storage.put_mut(Output("Hi".to_string()));
    let node_storage = node_storage.into_immutable();
    dbg!(Movement::move_node(&node_storage));
